...
namespace NPP.TaskTimers
{
    public static class TaskTimer
    {
        ...
        public static TaskDelay CreateTask(float delay, System.Action onDoneWaitDelay = null, int interval = 1, System.Action<int> onInterval = null, bool immediateRun = true)
        {
            if (!_initialized)
                Initialize();

            return _manager.CreateTask(delay, onDoneWaitDelay, onInterval, interval, immediateRun, false);
        }
        /// <summary>
        ...
        /// <param name="immediateRun">Immediate run this task after creation, if set to false, run <see cref="TaskDelay.Run"/> on created <see cref="TaskDelay"/>.</param>
        /// <returns>Created <see cref="TaskDelay"/>.</returns>
        public static TaskDelay CreateTaskLoop(float delay, System.Action onDoneWaitDelay = null, System.Action<int> onInterval = null, bool immediateRun = true)
        {
            if (!_initialized)
                Initialize();

            return _manager.CreateTask(delay, onDoneWaitDelay, onInterval, 0, immediateRun, true);
        }
        /// <summary>
        ...
        /// <param name="immediateRun">Immediate run this task after creation, if set to false, run <see cref="TaskDelay.Run"/> on created <see cref="TaskDelay"/>.</param>
        /// <returns>Created <see cref="TaskDelay"/>.</returns>
        public static TaskDelay CreateConditionalTask(float delay, System.Func<bool> condition, System.Action<int> onEachInterval = null, System.Action onConditionTrue = null, bool immediateRun = true)
        {
            if (!_initialized)
                Initialize();

            return _manager.CreateConditionalTask(delay, condition, onConditionTrue, onEachInterval, immediateRun);
        }
        /// <summary>
